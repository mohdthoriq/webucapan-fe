...
interface MenuCategoriesBulkDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onConfirm: (ids: string[]) => void
  selectedRows: MenuCategory[]
  isLoading?: boolean
}

export function MenuCategoriesBulkDeleteDialog({
  open,
  onOpenChange,
  onConfirm,
  selectedRows,
  isLoading,
}: MenuCategoriesBulkDeleteDialogProps) {
    ...
    <ConfirmDialog
      ...
      title={
        <div className='flex items-center gap-2'>
          ...
          <span>Hapus Multiple Kategori Menu</span>
        </div>
      }
      destructive
      ...
      confirmText='Hapus Kategori Menu'
      desc={
        <div className='mt-2 space-y-4 text-start'>
          ...
          >
            ...
            <AlertDescription className='text-xs opacity-90'>
              Anda akan menghapus{' '}
              <strong>{selectedRows.length} kategori menu</strong>. Tindakan ini

...
export function MenusTable({ search, navigate }: DataTableProps) {
  const { menusData, pagination: serverPagination, isLoading } = useMenus()
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableEmpty colSpan={menusColumns.length} />
            )}
          </TableBody>
        </Table>
      </div>
...
function TableRows({ table }: { table: TanstackTable<Menu> }) {
  return (
    <>

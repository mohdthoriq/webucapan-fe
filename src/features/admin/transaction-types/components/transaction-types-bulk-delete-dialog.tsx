...
interface TransactionTypesBulkDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onConfirm: (ids: string[]) => void
  selectedRows: TransactionType[]
  isLoading?: boolean
}

export function TransactionTypesBulkDeleteDialog({
  open,
  onOpenChange,
  onConfirm,
  selectedRows,
  isLoading,
}: TransactionTypesBulkDeleteDialogProps) {
    ...
    <ConfirmDialog
      ...
      title={
        <div className='flex items-center gap-2'>
          ...
          <span>Hapus Multiple Tipe Transaksi</span>
        </div>
      }
      destructive
      ...
      confirmText='Hapus Tipe Transaksi'
      desc={
        <div className='mt-2 space-y-4 text-start'>
          ...
          >
            ...
            <AlertDescription className='text-xs opacity-90'>
              Anda akan menghapus{' '}
              <strong>{selectedRows.length} tipe transaksi</strong>. Tindakan

...
interface AccountCategoriesBulkDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onConfirm: (ids: string[]) => void
  selectedRows: AccountCategory[]
  isLoading?: boolean
}

export function AccountCategoriesBulkDeleteDialog({
  open,
  onOpenChange,
  onConfirm,
  selectedRows,
  isLoading,
}: AccountCategoriesBulkDeleteDialogProps) {
    ...
    <ConfirmDialog
      ...
      title={
        <div className='flex items-center gap-2'>
          ...
          <span>Hapus Multiple Kategori Akun</span>
        </div>
      }
      destructive
      ...
      confirmText='Hapus Kategori Akun'
      desc={
        <div className='mt-2 space-y-4 text-start'>
          ...
          >
            ...
            <AlertDescription className='text-xs opacity-90'>
              Anda akan menghapus{' '}
              <strong>{selectedRows.length} kategori akun</strong>. Tindakan ini

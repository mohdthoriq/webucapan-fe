...
import { useBulkDeleteUnitMutation } from '../hooks/use-units-mutation'

interface UnitsBulkDeleteDialogProps {
  open: boolean
...
export function UnitsBulkDeleteDialog({
  open,
  onOpenChange,
  selectedRows,
  onSuccess,
}: UnitsBulkDeleteDialogProps) {
  const [confirmValue, setConfirmValue] = useState('')
  const deleteMutation = useBulkDeleteUnitMutation()
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} satuan yang dipilih. Satuan yang tidak

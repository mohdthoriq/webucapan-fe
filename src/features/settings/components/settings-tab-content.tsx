      ...
      {settingsTabs.map((tab) => (
        ...
        >
          <div className='mb-6 flex items-center gap-2'>
            ...
            <h2 className='text-xl font-semibold tracking-tight'>{tab.name}</h2>
          </div>
          ...
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
            {tab.items.map((item) => (
              <div
                key={item.title}
                className='group bg-background hover:border-primary/50 flex min-h-50 cursor-pointer flex-col items-center justify-center gap-4 rounded-xl border p-6 text-center shadow-sm transition-all hover:shadow-md'
              >

...
import { useBulkDeleteCompanyRoleMutation } from '../hooks/use-company-roles-mutation'

interface CompanyRolesBulkDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  selectedRows: CompanyRole[]
  onSuccess?: () => void
}

export function CompanyRolesBulkDeleteDialog({
  open,
  onOpenChange,
  selectedRows,
  onSuccess,
}: CompanyRolesBulkDeleteDialogProps) {
  const [confirmValue, setConfirmValue] = useState('')
  const deleteMutation = useBulkDeleteCompanyRoleMutation()

  const summary = useMemo(() => {
    const deletableCount = selectedRows.filter(
      (item) => item.is_deletable !== false && item.is_default !== true
    ).length
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} peran yang dipilih. Peran yang tidak
            bisa dihapus atau merupakan default akan diabaikan.
          </p>

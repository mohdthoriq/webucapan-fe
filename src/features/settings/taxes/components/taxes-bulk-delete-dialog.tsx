...
import { useBulkDeleteTaxMutation } from '../hooks/use-taxes-mutation'

interface TaxesBulkDeleteDialogProps {
  open: boolean
...
export function TaxesBulkDeleteDialog({
  open,
  onOpenChange,
  selectedRows,
  onSuccess,
}: TaxesBulkDeleteDialogProps) {
  const [confirmValue, setConfirmValue] = useState('')
  const deleteMutation = useBulkDeleteTaxMutation()
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} pajak yang dipilih. Pajak yang tidak
            bisa dihapus akan diabaikan.
          </p>

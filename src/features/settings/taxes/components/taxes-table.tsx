...
import { TaxesBulkDeleteDialog } from './taxes-bulk-delete-dialog'
...
export function TaxesTable({ search, navigate }: DataTableProps) {
  ...
  return (
    ...
    >
      ...
      <DataTableBulkActions table={table} entityName='pajak'>
        <Tooltip>
      ...
      {table.getRowModel().rows.map((row) => (
        ...
        >
          {row.getVisibleCells().map((cell) => (
            <TableCell
              onClick={() => {
                if (cell.column.id === 'select' || cell.column.id === 'status')
                  return
                setOpen('edit')

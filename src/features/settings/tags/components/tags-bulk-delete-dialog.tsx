...
import { useBulkDeleteTagMutation } from '../hooks/use-tags-mutation'

interface TagsBulkDeleteDialogProps {
  open: boolean
...
export function TagsBulkDeleteDialog({
  open,
  onOpenChange,
  selectedRows,
  onSuccess,
}: TagsBulkDeleteDialogProps) {
  const [confirmValue, setConfirmValue] = useState('')
  const deleteMutation = useBulkDeleteTagMutation()
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} tag yang dipilih. Tag yang tidak bisa
            dihapus akan diabaikan.
          </p>

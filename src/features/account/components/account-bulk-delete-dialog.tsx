...
export function AccountBulkDeleteDialog({
  ...
  const summary = useMemo(() => {
    const deletableCount = selectedRows.filter(
      (account) => account.is_deletable === true
    ).length
    ...
    return {
      ...
      deletableIds: selectedRows
        .filter((account) => account.is_deletable === true)
        .map((i) => i.id),
    }
  }, [selectedRows])
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} akun yang dipilih, akun default dan akun
            yang sudah memiliki transaksi tidak dapat dihapus
          </p>

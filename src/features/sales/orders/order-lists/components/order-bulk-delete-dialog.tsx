...
export function OrderBulkDeleteDialog({
  ...
  const summary = useMemo(() => {
    // For Sales Orders, let's assume those in 'draft' status are deletable.
  ...
  return (
    <ConfirmDialog
      ...
      title='Hapus Multiple Sales Order'
      destructive
      ...
      desc={
        <div className='mt-2 space-y-4 text-start'>
          <p className='text-muted-foreground text-sm'>
            Hapus {selectedRows.length} sales order yang dipilih. Order yang sudah

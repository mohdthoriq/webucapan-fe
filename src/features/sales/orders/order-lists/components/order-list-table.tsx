...
export function OrderListsTable({ search, navigate }: DataTableProps) {
  ...
  const hasPermission = useHasPermission(PERMISSION_KEY.SALES_ORDER_DELETE)
  const { bulkDeleteMutation } = useSalesOrderMutation()
  ...
  const {
    ...
    columnFilters: [
      { columnId: 'Nomor Order', searchKey: 'order_number', type: 'string' },
    ],
  })
  ...
  return (
    <div className='flex flex-1 flex-col gap-4'>
      ...
      <DataTableBulkActions table={table} entityName='sales order'>
        <Tooltip>
...
function TableRows({ table }: { table: TanstackTable<SalesOrder> }) {

...
export const OrderItemRow = memo(function OrderItemRow({
  ...
  index: number
  form: ReturnType<
    typeof useForm<CreateSalesOrderFormData | UpdateSalesOrderFormData>
  >
  ...
  useEffect(() => {
    const currentTotal = form.getValues(
      `sales_order_items.${index}.line_total`
    )
    if (currentTotal !== rowTotal) {
      form.setValue(`sales_order_items.${index}.line_total`, rowTotal)
    }
  }, [rowTotal, form, index])
  ...
  return (
    <TableRow>
      <TableCell className='px-2 py-2'>
        <FormField
          control={form.control}
          name={`sales_order_items.${index}.product_id`}
          render={({ field }) => (
              ...
              <OrderFormCombobox
                ...
                onValueChange={(value) => {
                  ...
                  if (value) {
                    ...
                    if (product) {
                      form.setValue(
                        `sales_order_items.${index}.unit_price`,
                        Number(product.sale_price)
                      )
                      form.setValue(
                        `sales_order_items.${index}.purchase_price`,
      ...
      <TableCell className='px-2 py-2'>
        <FormField
          control={form.control}
          name={`sales_order_items.${index}.description`}
          render={({ field }) => (
            <FormItem className='space-y-0'>
              <FormControl>
      ...
      <TableCell className='px-2 py-2'>
        <FormField
          control={form.control}
          name={`sales_order_items.${index}.unit_price`}
          render={({ field }) => (
            <FormItem className='space-y-0'>
              <FormControl>
      ...
      <TableCell className='px-2 py-2'>
        <FormField
          control={form.control}
          name={`sales_order_items.${index}.discount`}
          render={({ field }) => (
            <FormItem className='space-y-0'>
              <FormControl>
                <Input
      ...
      <TableCell className='px-2 py-2'>
        <FormField
          control={form.control}
          name={`sales_order_items.${index}.tax_id`}
          render={({ field }) => (
            <FormItem className='space-y-0'>
              <Select
                onValueChange={(value) => {

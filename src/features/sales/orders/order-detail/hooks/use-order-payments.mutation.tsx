...
export function useSalesOrderMutation() {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: () => {
      toast.loading('Menghapus Sales Order...', { id: 'so-delete-toast' })
    },
    onSuccess: async () => {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      const response = await apiClient.post(`/sales-orders/bulk-delete`, { ids })
      return response.data
    },
    onMutate: () => {
...
export function useCreateSalesOrderPaymentMutation() {
  const queryClient = useQueryClient()
  return useMutation({

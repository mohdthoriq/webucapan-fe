...
export const calculateTotals = (
  form: Pick<
    ReturnType<typeof useForm<CreatePurchasesOrderFormData | UpdatePurchasesOrderFormData>>,
    'setValue' | 'getValues'
  >,
  items: (CreatePurchasesOrderFormData | UpdatePurchasesOrderFormData)['purchase_order_items'],
  additionalDiscounts: (
  ...
  items.forEach((item) => {
    ...
    if (item.tax_id) {
      ...
      if (tax) {
        ...
        const signedBarangTax = tax.is_withholding ? -itemTax : itemTax
        newTaxTotal += signedBarangTax

        if (tax.name) {
          taxBreakdown[tax.name] = (taxBreakdown[tax.name] || 0) + signedBarangTax
        }
      }
    }
  })

...
export const purchasesOrderItemSchema = z.object({
...
export type PurchasesOrderItemFormData = z.infer<
  typeof purchasesOrderItemSchema
>

export const purchasesOrderItemUpdateSchema = z.object({
  id: z.uuid('ID produk tidak boleh kosong').optional().nullable(),
  product_id: z.string().min(1, 'Produk tidak boleh kosong'),
  description: z.string().optional(),
  quantity: z.number().positive(),
...
export type PurchasesOrderItemUpdateFormData = z.infer<
  typeof purchasesOrderItemUpdateSchema
>

export const CreatePurchasesOrderSchema = z
  .object({
    ...
    order_number: z.string().min(1, 'Nomor pesanan tidak boleh kosong'),
    order_date: z.date(),
    due_date: z.date(),
    ...
    purchase_order_items: z
      .array(purchasesOrderItemSchema)
  ...
  .refine((data) => data.order_date <= data.due_date, {
    message: 'Tanggal jatuh tempo harus lebih dari tanggal pesanan',
    path: ['due_date'],
  })
  .refine(
    (data) => {
...
export type CreatePurchasesOrderFormData = z.infer<
  typeof CreatePurchasesOrderSchema
>

export const UpdatePurchasesOrderSchema = z
  .object({
    id: z.uuid().min(1, 'ID pesanan tidak boleh kosong'),
    vendor_id: z.string().min(1, 'Vendor tidak boleh kosong'),
    shift_id: z.string().optional().nullable(),
    payment_term_id: z.string().optional().nullable(),
    order_number: z.string().min(1, 'Nomor pesanan tidak boleh kosong'),
    ...
    payment_status: z.enum(['unpaid', 'partially_paid', 'paid']),
    purchase_order_items: z
      .array(purchasesOrderItemUpdateSchema)
      .min(1, 'Pesanan harus memiliki minimal 1 item'),
    tags: z.array(z.string()).nullable(),
...
export type UpdatePurchasesOrderFormData = z.infer<
  typeof UpdatePurchasesOrderSchema
>

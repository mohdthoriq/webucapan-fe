...
import type { PaginationApiResponse, PurchasesOrder } from '@/types'
import apiClient from '@/lib/api-client'
import { QUERY_KEY } from '@/constants/query-key'

export interface PurchasesOrderListQueryParams {
  page?: number
  limit?: number
  order?: string
  company_id?: string
...
export function usePurchasesOrderListQuery(params?: PurchasesOrderListQueryParams) {
  return useQuery({
    queryKey: [QUERY_KEY.SALES, QUERY_KEY.PURCHASES_ORDER, params],
    queryFn: async () => {
      ...
      const url = queryParams.toString()
        ...
        : '/purchase-orders'
      const response = await apiClient.get<PaginationApiResponse<PurchasesOrder>>(url)

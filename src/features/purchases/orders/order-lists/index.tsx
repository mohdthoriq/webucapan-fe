...
import { PurchasesOrdersTablePrint } from './components/print/purchase-orders-table-print'
import type { PurchasesOrderListQueryParams } from './hooks/use-purchase-order-list-query'

const route = getRouteApi('/_authenticated/purchases/orders/')
...
function OrderListsContent() {
  ...
  return (
    <PermissionGuard
      permission={PERMISSION_KEY.PURCHASE_ORDER}
      fallback={
        <OrderListFallback search={search} navigate={navigate} />
      }
    >
      <Card className='relative'>
        <CardHeader>
          <div className='flex justify-between'>
            ...
            <div className='flex flex-col items-end gap-2 md:flex-row md:items-start'>
              <Button onClick={() => navigate({ to: '/purchases/orders/add' })}>
        ...
        >
          <PurchasesOrdersTablePrint ref={printRef} />
        </div>
      </Card>
    </PermissionGuard>
  )
...
function OrderLists() {
  ...
  const queryParams: PurchasesOrderListQueryParams = {
    ...
    payment_status: (search.payment_status as PurchasesOrderListQueryParams['payment_status']) || undefined,
    document_status: (search.document_status as PurchasesOrderListQueryParams['document_status']) || undefined,
    ...
    date_from: search.date_from ? new Date(search.date_from as string) : undefined,
    date_to: search.date_to ? new Date(search.date_to as string) : undefined,
    due_date_from: search.due_date_from ? new Date(search.due_date_from as string) : undefined,
    due_date_to: search.due_date_to ? new Date(search.due_date_to as string) : undefined,
    payment_date_from: search.payment_date_from ? new Date(search.payment_date_from as string) : undefined,
    payment_date_to: search.payment_date_to ? new Date(search.payment_date_to as string) : undefined,

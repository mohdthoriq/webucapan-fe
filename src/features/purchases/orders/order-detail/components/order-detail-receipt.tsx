...
import { usePrintPurchasesOrderQuery } from '../hooks/use-print-purchase-order-query'
...
export function OrderDetailReceipt({ order }: OrderDetailReceiptProps) {
  const { refetch, isFetching: isPrinting } = usePrintPurchasesOrderQuery(
    order.id
  )
      ...
      <CardContent className='px-4 pt-4'>
                ...
                >
                  ...
                  <TableCell className='px-2 py-1.5 align-top'>
                    <p className='text-[13px] font-medium'>
                      {formatCurrency(Number(item.unit_price), order.currency)}
                    </p>
                  </TableCell>
        ...
        <div className='mt-6 flex flex-col items-start justify-between gap-6 md:flex-row'>
          ...
          <div className='bg-muted/20 w-full space-y-1.5 rounded-lg p-4 md:w-96'>
            ...
            {order.purchase_order_items?.filter((item) => item.tax).length >
              0 &&
              Object.entries(
            ...
            {Number(order.tax_total) > 0 &&
              order.purchase_order_items?.every((item) => !item.tax) && (

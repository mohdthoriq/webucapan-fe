...
import { useSalesPerProductQuery } from './hooks/use-sales-per-product-query'
...
export default function SalesPerProductPage() {
  ...
  const { data, isLoading } = useSalesPerProductQuery({
  ...
  const handleCategoryChange = (val: string | undefined) => {
    navigate({
      search: (prev) => ({
        ...prev,
        product_category_id: val || undefined,
        page: 1,
      }),
    })
  }
  ...
  return (
    <PermissionGuard permission={PERMISSION_KEY.REPORTS_SALES_PRODUCT_VIEW}>

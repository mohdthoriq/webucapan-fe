...
interface ExpenseListsTablePrintProps {
  className?: string
}

export const ExpenseListsTablePrint = forwardRef<
  HTMLDivElement,
  ExpenseListsTablePrintProps
>(({ className }, ref) => {
  const company = useAuthStore((state) => state.auth.user?.company)
  const { expensesListsData, columnVisibility } = useExpensesLists()

  // Filter out 'actions' and hidden columns
  const visibleColumns = expensesListsColumns.filter((col) => {
    ...
    >
      ...
      <div className='w-full overflow-hidden rounded-md border border-slate-300'>
        <table className='w-full border-collapse text-left text-[10px]'>
          ...
          <tbody>
            {expensesListsData.length > 0 ? (
              expensesListsData.map((invoice) => (
                <tr
                  key={invoice.id}
...
ExpenseListsTablePrint.displayName = 'ExpenseListsTablePrint'

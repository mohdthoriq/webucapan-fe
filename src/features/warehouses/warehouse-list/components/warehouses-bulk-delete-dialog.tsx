...
interface WarehousesBulkDeleteDialogProps {
  open: boolean
...
export function WarehousesBulkDeleteDialog({
  open,
  onOpenChange,
  onConfirm,
  selectedRows,
  isLoading,
}: WarehousesBulkDeleteDialogProps) {
  ...
  return (
    <ConfirmDialog
      ...
      onOpenChange={(v) => {
        onOpenChange(v)
        if (!v) setConfirmValue('') // Reset input saat dialog ditutup
      }}
      title={
        <div className='flex items-center gap-2'>
          <Trash2 className='text-destructive h-5 w-5' />
          <span>Hapus Multiple Gudang</span>
        </div>
      }
      destructive
        ...
        <div className='mt-2 space-y-4 text-start'>
          ...
          >
            ...
            <AlertDescription className='text-xs opacity-90'>
              Anda akan menghapus <strong>{selectedRows.length} gudang</strong>.
              Tindakan ini permanen dan tidak dapat dibatalkan.
            </AlertDescription>
          </Alert>

          {/* Kotak Konfirmasi Input */}

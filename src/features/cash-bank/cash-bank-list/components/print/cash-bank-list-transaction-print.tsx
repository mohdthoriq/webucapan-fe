...
interface CashBankListsTablePrintProps {
  className?: string
}

export const CashBankListsTablePrint = forwardRef<
  HTMLDivElement,
  CashBankListsTablePrintProps
>(({ className }, ref) => {
  const company = useAuthStore((state) => state.auth.user?.company)
  const { cashBankTransactionListsData, columnVisibility } = useCashBankLists()

  // Filter out 'actions' and hidden columns
  const visibleColumns = cashBankListsColumns.filter((col) => {
    ...
    >
      ...
      <div className='w-full overflow-hidden rounded-md border border-slate-300'>
        <table className='w-full border-collapse text-left text-[10px]'>
          ...
          <tbody>
            {cashBankTransactionListsData?.length > 0 ? (
              cashBankTransactionListsData?.map((cashBank) => (
                <tr
                  key={cashBank.id}
                ...
                >
                  {visibleColumns.map((col, colIndex) => (
                    ...
                    >
                      {flexRender(col.cell, {
                        row: { original: cashBank } as any,
                        column: col as any,
                        table: {} as any,
                        cell: {} as any,
                        getValue: () =>
                          (cashBank as any)[(col as any).accessorKey],
                        renderValue: () =>
                          (cashBank as any)[(col as any).accessorKey],
                      } as any)}
                    </td>
...
CashBankListsTablePrint.displayName = 'CashBankListsTablePrint'

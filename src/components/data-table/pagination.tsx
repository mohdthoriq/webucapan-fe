  ...
  return (
    ...
    >
      ...
      <div className='flex items-center sm:space-x-6 lg:space-x-8'>
        <div className='flex w-[100px] items-center justify-center text-sm font-medium @max-3xl/content:hidden whitespace-nowrap'>
          Halaman {currentPage} dari {totalPages}
        </div>
          ...
          {pageNumbers.map((pageNumber, index) => (
            <div key={`${pageNumber}-${index}`} className='flex items-center'>
              {pageNumber === '...' ? (
                ...
                >
                  <span className='sr-only'>Pergi ke halaman {pageNumber}</span>
                  {pageNumber}
                </Button>
              )}
            </div>
